...
async function validate(username,email,phone){
    ...
    let userFound = await db.get().collection('users').findOne({$or:[{username:username},{email:email},{phone:phone}]})
...
module.exports= {
    ...
    deleteUser:(id)=>{
        return new Promise(async (resolve,reject)=>{
            let users = await db.get().collection('users').deleteOne({_id:objectId(id)}).then((returnVal)=>{
                resolve(returnVal)
            })
        })
    }
